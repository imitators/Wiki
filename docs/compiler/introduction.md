...
**符号表** (symbol table) 是用于语言翻译器中的结构，在编译过程中使用。
...
> x86 体系结构具有 CISC 指令集，不过仍然受到了 RISC 的很大影响。
>
> x86 机器需要高性能时，仍然使用 RISC 指令集。

#### RISC

RISC (Reduced Instruction-Set Computer, 精简指令集计算机) 通过简单指令消灭复杂指令之间的冗余，使得编译器和硬件能更好的优化。

大部分通用处理器架构都是 RISC，例如 PowerPC、SPARC、MIPS、Alpha 和 PA-RISC。

#### 专用体系结构

以下体系结构概念的提出，都促进了编译器的发展和研究：
...
以上列举的部分思想已经应用于嵌入式机器的设计。

结果是通用处理器的结构由于规模经济而趋同，而专用处理器则体现了结构的多样性。

人们可以针对特定应用裁剪出专用处理器，以获得更好的费效比。
...
二进制翻译指将一个机器的二进制代码翻译另一个机器的二进制代码，使得一个机器能运行另一个指令集编译的软件。

二进制翻译也可以用来提供向后兼容性。

#### 硬件合成

硬件设计使用高级硬件描述语言，例如 Verilog 和 VHDL。

硬件设计通常在 RTL (Register Transfer Level, 寄存器传输层) 上描述的。
